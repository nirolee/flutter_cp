...
import 'package:chucp/common/event/code.dart';
///http请求管理类，可单独抽取出来
class HttpRequest {
  ...
  static put(url,param) async{
    return await request(_baseUrl+url, param, null, new Options(method: "PUT", contentType: "text/plain; charset=utf-8"));
  }
  ...
  static request(url, params, Map<String, String> header, Options option, {noTip = false}) async {
    ...
    //key 
    headers['X-LC-Id'] = Config.XLCID;
    headers['X-LC-Key'] = Config.XLCKEY;
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        return ResultData(response.data, true, 0, headers: response.headers);
      }
    } catch (e) {
